...
void SimulatorBase::Generator_Init()
{
	//const int rem_err = system("rm -rf data");
...
void SimulatorBase::Generator_Input()
{
	TimeManager *tm = TimeManager::getCurrent();
	if (skipTime <= tm->getTime())
	{
		...
		sampleFile.open("./data/sample_" + filename + "_" + std::to_string(tm->getTime()) + ".bin",std::ios::binary | std::ios::out);
		...
		for (unsigned int m = 0; m < nModels; m++)
		{
			...
			std::vector<std::pair<unsigned int, std::vector<float>>> tdata;
...
#pragma omp for schedule(static)
				for (int i = 0; i < (int)numParticles; i++)
				{
					...
					features.push_back(fm->getPosition(i)[0]);
					features.push_back(fm->getPosition(i)[1]);
					features.push_back(fm->getPosition(i)[2]);

					features.push_back(fm->getVelocity(i)[0]);
					features.push_back(fm->getVelocity(i)[1]);
					features.push_back(fm->getVelocity(i)[2]);

					features.push_back(0.0f);

#pragma omp critical
					{
						tdata.push_back(std::pair<unsigned int, std::vector<float>>(i, features));
					}
					features.clear();
				}
			}
			fluid_data.push_back(std::pair<unsigned int, std::vector<std::pair<unsigned int, std::vector<float>>>>(m, tdata));
			tdata.clear();
		}

		const unsigned int SolidModels = sim->numberOfBoundaryModels();

		for (unsigned int m = 0; m < SolidModels; m++)
		{
			BoundaryModel_Akinci2012 *fm = static_cast<BoundaryModel_Akinci2012 *>(sim->getBoundaryModel(m));
			const unsigned int numParticles = fm->numberOfParticles();
...
#pragma omp for schedule(static)
				for (int i = 0; i < (int)numParticles; i++)
				{
					...
					//if(fabs(fm->getVelocity(i)[0]) > 0.0f && fabs(fm->getVelocity(i)[1]) > 0.0f && fabs(fm->getVelocity(i)[2]) > 0.0f)
					//{
					features.push_back(1.0f);
					//}
					//else
					//{
						//features.push_back(2.0f);
					//}
...
#pragma omp critical
			...
			solid_data.push_back(std::pair<unsigned int, std::vector<std::pair<unsigned int, std::vector<float>>>>(m, tdata));
			tdata.clear();
		}
	}
}
void SimulatorBase::Generator_Output()
{
	TimeManager *tm = TimeManager::getCurrent();
	if (skipTime <= tm->getTime())
	{
		...
		for (unsigned int i = 0; i < fluid_data.size(); i++)
		{
			unsigned int fluidmodel_id = fluid_data[i].first;
			FluidModel *fm = sim->getFluidModel(fluidmodel_id);

			for (unsigned int j = 0; j < fluid_data[i].second.size(); j++)
			{
				unsigned int particle_id = fluid_data[i].second[j].first;

				Vector3r outvelocity = fm->getVelocity(particle_id);

				fluid_data[i].second[j].second.push_back(outvelocity[0]);
				fluid_data[i].second[j].second.push_back(outvelocity[1]);
				fluid_data[i].second[j].second.push_back(outvelocity[2]);
				
				sampleFile.write(reinterpret_cast<char *>(&(fluid_data[i].second[j].second[0])), fluid_data[i].second[j].second.size() * sizeof(fluid_data[i].second[j].second[0]));

			}
		}
		fluid_data.clear();

		for (unsigned int i = 0; i < solid_data.size(); i++)
		{
			unsigned int solidmodel_id = solid_data[i].first;
			BoundaryModel_Akinci2012 *fm = static_cast<BoundaryModel_Akinci2012 *>(sim->getBoundaryModel(solidmodel_id));

			for (unsigned int j = 0; j < solid_data[i].second.size(); j++)
			{
				unsigned int particle_id = solid_data[i].second[j].first;

				Vector3r outvelocity = fm->getVelocity(particle_id);

				solid_data[i].second[j].second.push_back(outvelocity[0]);
				solid_data[i].second[j].second.push_back(outvelocity[1]);
				solid_data[i].second[j].second.push_back(outvelocity[2]);

				sampleFile.write(reinterpret_cast<char *>(&(solid_data[i].second[j].second[0])), solid_data[i].second[j].second.size() * sizeof(solid_data[i].second[j].second[0]));
			}
		}
		solid_data.clear();
	}

	std::cout << tm->getTime() << std::endl;
}

void SimulatorBase::Generator_Close()
{
	sampleFile.close();
}
