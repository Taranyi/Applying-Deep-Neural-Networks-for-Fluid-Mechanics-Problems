...
class FeatureNet(object):
    def __init__(self, RANDOM_NUMBER, VOXEL_NUMBER, mini_max_particles_voxel, max_particles_voxel, grid_size):
        ...
        #self.loss = tf.reduce_sum(loss_full) / tf.cast(tf.shape(loss_full)[0], tf.float32)
    ...
    def trans(self, RANDOM_NUMBER, VOXEL_NUMBER, max_particles_voxel, grid_size):
        ...
        assert grid_size[0] == 11
        assert grid_size[1] == 11
        assert grid_size[2] == 11
        
        tensor_4D = tf.reshape(max128, [1, 11, 11, 11, 128])
        ...
        conv5 = tf.layers.conv3d(conv4, 32, [3, 3, 3], [3, 3, 3], padding="same", name="conv3d_3", activation=tf.nn.tanh,
                                      kernel_initializer=tf.random_normal_initializer,
                                      bias_initializer=tf.random_normal_initializer)  # [1, 3, 3, 3, 32]
        #conv6 = tf.layers.conv3d(conv5, 32, [3, 3, 3], [3, 3, 3], padding="same", name="conv3d_4", activation=tf.nn.tanh,kernel_initializer=tf.random_normal_initializer,bias_initializer=tf.random_normal_initializer)  # [1, 1, 1, 1, 32]
        
        dim = tf.shape(particles_with_knn)[0]
        final_feature = tf.concat([particles_with_knn, tf.tile(tf.reshape(conv5, [1, 32]), [dim, 1])],
                                       axis=1)  # [-1, 48]
...
class Model(object):
    ...
    def train(self, session, data, train=False, summary=False, onlypred=False):
        ...
        input_feed[self.voxel_index[0]] = data[3]#[0]  # [N]
        input_feed[self.feature[0]] = data[0]#[0]  # [N, 7]
        input_feed[self.neighbor[0]] = data[1]#[0]  # [N, K]
        input_feed[self.labels[0]] = data[2]#[0]  # [N, 3]
