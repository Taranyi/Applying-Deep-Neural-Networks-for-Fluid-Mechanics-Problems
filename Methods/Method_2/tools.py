...
def Find_csv(dir, fps_range=None):
    ...
    if fps_range is None:
        for item in items:
            if item.split('/')[-1].split('.')[-1] == 'bin':
                csv_files.append(item)
    elif fps_range[0] == fps_range[1]:
        return [os.path.join(dir, 'all_particles_' + str(fps_range[0]) + '.bin')]
    else:
        for item in items:
            if item.split('/')[-1].split('.')[-1] == 'bin':
...
def Load_csv(filename):
    with open(filename) as f:
        ...
        #data = np.fromfile(f, dtype=np.float32).reshape(-1,10)
        return np.fromfile(f, dtype=np.float32).reshape(-1,10)  # input[:, :7], output[:, 7:]


def Draw_voxels(point_cloud, voxel_size, grid_size, lidar_coord):
    shifted_coord = point_cloud[:, :3] + lidar_coord
    voxel_index = np.floor(abs(shifted_coord[:, :]) / voxel_size).astype(np.int)  # int lower than num
...
def build_mini_voxel(MINI_VOXEL_NUMBER, mini_max_particles_voxel, mini_voxel_size, data, voxel_index, bias_voxel):
    ...
    #print('Mean neighbor:',sums/num)
    #print('count of voxels which contains too many particles:',count)
