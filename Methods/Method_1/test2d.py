...
def find_neighbor(X,neighbor_number,rad):
	...
	ind = tree.query_radius(X[:,:2], r=rad)  
...
class FluidNet(tf.keras.Model):
	...
	def __init__(self):
		...
		self.fcn1 = tf.keras.layers.Dense(27, input_shape=(5,), kernel_initializer=tf.random_normal_initializer())
		...
		self.gl1 = tf.keras.layers.GaussianNoise(0.001,input_shape=(4,))
		self.gl2 = tf.keras.layers.GaussianNoise(0.001,input_shape=(5,))
		#self.d1 = tf.keras.layers.Dropout(.2)
		#self.d2 = tf.keras.layers.Dropout(.2)
		#self.d3 = tf.keras.layers.Dropout(.2)
		
		
		self.fcn5 = tf.keras.layers.Dense(2, kernel_initializer=tf.random_normal_initializer())
	...
	def call(self, inputs):
		...
		x = tf.nn.leaky_relu((self.fcn1(self.gl2(neighbor))))
		x = tf.reduce_mean(x, axis=1, name="mean")
		x = tf.concat([self.gl1(feature), x], axis=1)
...
if train == True :
	...
	for e in range(epoch):
		...
		for idx in selected_files[:5]:
			...
			train_history = fluidnet.fit([x1_tr,x2_tr,y_tr],y_tr,epochs=1,batch_size=256,shuffle=True,callbacks=[],validation_data=([x1_te,x2_te,y_te],y_te),validation_freq=1)
