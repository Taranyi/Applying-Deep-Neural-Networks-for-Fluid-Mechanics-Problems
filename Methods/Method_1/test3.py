...
def build_input(path,files,indexes):
    ...
    for i in indexes:
	    ...
	    final_label.append(1000*(data[:, 7:]-data[:,3:6]))
...
class FluidNet(tf.keras.Model):
	...
	def __init__(self):
		...
		self.fcn1a = tf.keras.layers.Dense(32, input_shape=(7,),activation=tf.nn.leaky_relu, kernel_initializer=tf.random_normal_initializer())
		#self.fcn1b = tf.keras.layers.Dense(16,activation=tf.nn.leaky_relu, kernel_initializer=tf.random_normal_initializer())
		...
		self.fcn2 = tf.keras.layers.Dense(32, activation=tf.nn.leaky_relu,kernel_initializer=tf.random_normal_initializer())
		
		self.fcn3 = tf.keras.layers.Dense(16, activation=tf.nn.leaky_relu,kernel_initializer=tf.random_normal_initializer())
		
		self.fcn4 = tf.keras.layers.Dense(8, activation=tf.nn.leaky_relu,kernel_initializer=tf.random_normal_initializer())
...
if train == True :
	...
	for e in range(epoch):
		...
		for idx in selected_files:
			...
			train_history = fluidnet.fit([x1_tr,x2_tr,y_tr],y_tr,epochs=100,batch_size=256,shuffle=True,callbacks=[],validation_data=([x1_te,x2_te,y_te],y_te),validation_freq=1)
